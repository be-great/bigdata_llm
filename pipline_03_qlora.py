...
# 1) Prepare small supervised dataset from Parquet (Spark -> pandas -> HF Dataset)
spark = SparkSession.builder.appName("MakeSFTData").getOrCreate()
pdf = spark.read.parquet(args.curated)\
        .select("reason_for_visit","treatment_description")\
        .na.drop()\
        .limit(20000).toPandas()
spark.stop()

# 1) Formats each example like a Q&A.
def fmt(r):
    return {
      "text": f"<s>[INSTRUCTION] {r['reason_for_visit']}\n[RESPONSE] {r['treatment_description']}</s>"
    }
train_data = [fmt(r) for _, r in pdf.iterrows()]
dataset = Dataset.from_list(train_data)

# 2) Load base model quantized (4-bit) for QLoRA
...
# 3) LoRA config (QLoRA: LoRA on top of 4-bit)
lconf = LoraConfig(
    r=16, lora_alpha=32, lora_dropout=0.05,
    bias="none", task_type="CAUSAL_LM",
    target_modules="all-linear"  # general setting that works across archs
)
...
# 6) Save adapter only (base model remains from HF cache)
# save only the medical knowledge not the whole brain
